...
var gravityMode = "inverseSquare",  newColor=[]; // Nuevo: modo de gravedad
...
var darkMatterInteractionFactor = 0.01;
...
function setNewMass(t,n,d,c,pt){
    newMass=t;
    newName=n;
    newDensity=d;

    if(c===undefined){
        newColor=[255, Math.round(256/(1+Math.pow(t/1e5,1))), Math.round(256/(1+Math.pow(t/1e4,1)))];
    } else {
        newColor = c;
    }
...
function draw2() {
    context.clearRect(0, 0, width, height), context.beginPath(), context.moveTo(startCoords[0], startCoords[1]), context.lineTo(endCoords[0], endCoords[1]), (context.strokeStyle = "blue"), (context.strokeWidth = 2), context.stroke();
    for (var t = 0; t < particleList.length; t++) {
        ...
        if (e.radius < 3) context.fillStyle = "#" + o[3];
            ...
            r.addColorStop(0, "rgba(" + o[0] + "," + o[1] + "," + o[2] + ",1.0)"), r.addColorStop(1, "rgba(" + o[0] + "," + o[1] + "," + o[2] + ",0)"), (context.fillStyle = r);
        }
        context.fill();
    }
}
...
function integrate() {
    ...
    // Handle collisions and create new merged particles
    var newParticles = [];
    var particlesToRemove = new Set();
    ...
    for (var i = 0; i < particleList.length; i++) {
        ...
        for (var j = i + 1; j < particleList.length; j++) {
            ...
            if (dist < (p1.radius + p2.radius)) {
                // Calculate merged particle properties
                ...
                // Determine new type (prioritize special types)
                var newType = p1.type;
                if (p2.type === "blackhole" || p2.type === "neutron") {
                    newType = p2.type;
                }
                ...
                // Mark original particles for removal
    ...
    // Remove collided particles and add new merged ones
    particleList = particleList.filter(p => !particlesToRemove.has(p));
    particleList = particleList.concat(newParticles);
...
// Physics integration
function integrate3() {
    if(newType==='blackhole' || newType==='neutron'){
            // Calculate accelerations
    for (var i = 0; i < particleList.length; i++) {
        var p1 = particleList[i];
        p1.ax = 0;
        ...
        for (var o = particleList[e], r = 0, i = 0, s = 0; s < particleList.length; s++)
            if (otherParticle = particleList[s], o != otherParticle && !o.collided && !otherParticle.collided) {
                ...
                if (c < o.radius / 1.5 + otherParticle.radius / 1.5) {
                    o.collided = !0, otherParticle.collided = !0
                    var l = o.mass + otherParticle.mass;
                    // Calcula la densidad para la nueva partícula fusionada
                    // Una forma simple es promediar las densidades o recalcularla basada en la nueva masa y radio.
                    // Aquí, recalcularemos la densidad de la nueva partícula fusionada
                    var newParticleRadius = Math.log(Math.E + l / 1e3);
                    var newParticleDensity = l / (4 / 3 * Math.PI * Math.pow(newParticleRadius, 3));

                    var d = new Particle(l, (o.x * o.mass + otherParticle.x * otherParticle.mass) / l, (o.y * o.mass + otherParticle.y * otherParticle.mass) / l, (o.vx * o.mass + otherParticle.vx * otherParticle.mass) / l, (o.vy * o.mass + otherParticle.vy * otherParticle.mass) / l, newParticleDensity) // Pasamos la nueva densidad
                    t.push(d)
                }
                var p = otherParticle.mass / (c * c)
                r += p * a / c, i += p * n / c
