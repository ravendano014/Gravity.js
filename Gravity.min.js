        ...
        for (var j = 0; j < particleList.length; j++) {
            if (i !== j) {
                ...
                if (dist > 1e-5) {
                    ...
                    // Special handling for black holes and neutron stars
                    if (p2.isBlackHole || p2.type === "neutron") {
                        // Enhanced gravity near massive objects
                        if (dist < p2.gravityInfluenceRadius) {
                            // Stronger gravity close to the object
                            var influenceFactor = 1 + (p2.gravityInfluenceRadius - dist) / p2.gravityInfluenceRadius * 10;
                            force = G * p2.mass / distSq * influenceFactor;
                            
                            // Matter destruction when too close
                            if ((p2.isBlackHole && dist < p2.eventHorizonRadius) || 
                                (p2.type === "neutron" && dist < p2.radius * 5)) {
                                // Particle is absorbed/destroyed
                                p1.x = p2.x;
                                p1.y = p2.y;
                                p1.vx = p2.vx;
                                p1.vy = p2.vy;
                                
                                // Add to black hole/neutron star mass
                                ...
                                } else if (p2.type === "neutron") {
                                    p2.radius = Math.sqrt(p2.mass) / 100;
                                    p2.magnetosphereRadius = p2.radius * 15;
                                }
                                ...
                                if (Math.random() < 0.3) {
                                    var energyParticle = new Particle(
                                        p1.mass * 0.1, // 10% of mass converted to energy
                                        (p1.x + p2.x) / 2,
                                        (p1.y + p2.y) / 2,
                                        (p1.vx + p2.vx) / 2 + (Math.random() - 0.5) * 20,
                                        (p1.vy + p2.vy) / 2 + (Math.random() - 0.5) * 20,
                                        "matter",
                                        "Energy Burst"
                                    );
                                    ...
                                    particleList.push(energyParticle);
                                }
                                continue;
                            }
                        } else {
                    ...
                    // Resto de las interacciones gravitatorias normales...
