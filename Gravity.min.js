...
function placeEclipseScenario() {
    ...
    const G_sim = 0.001; // Ajusta la constante G para que las órbitas sean visibles en tu escala
    // Puedes cambiar G globalmente al principio de Gravity.min.js o aquí temporalmente
    // var G = 1; al principio del archivo
    // Si cambias G aquí, recuerda que afectará todas las demás simulaciones a menos que la resetees.
    // Para este ejemplo, asumiremos que G es la global, o la ajustamos antes de llamar a esta función.

    // Sol (cuerpo central y más masivo)
    const massSun = 1e6; // Masa muy grande
    particleList.push(new Particle(
        massSun,
        width / 2,
        height / 2,
    ...
    const massEarth = 1e3; // Masa mediana
    const distEarthSun = 200; // Distancia al Sol
    const velEarth = Math.sqrt(G * massSun / distEarthSun) * 0.7; // Velocidad para órbita (ajusta factor)
    particleList.push(new Particle(
        massEarth,
        width / 2 + distEarthSun,
        height / 2,
        0,
        velEarth,
        "matter",
    ...
    const massMoon = 1e1; // Masa pequeña
    const distMoonEarth = 30; // Distancia a la Tierra
    // Velocidad de la Luna relativa a la Tierra, más la velocidad orbital de la Tierra
    const velMoon = Math.sqrt(G * massEarth / distMoonEarth) * 0.9;
    particleList.push(new Particle(
        massMoon,
        width / 2 + distEarthSun + distMoonEarth,
        height / 2,
        0,
        velEarth + velMoon, // Suma la velocidad orbital de la Tierra
        "matter",
        "Luna"
    ));
