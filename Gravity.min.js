...
function init(){
    ...
    // visibleFactor determina la amplitud del mundo visible, para Sistema Solar 10  
    var visibleFactor = 1;

    t.width = (window.innerWidth - 30) * visibleFactor; // Ampliar mundo visible
    t.height = (window.innerHeight - 20) * visibleFactor; // Ampliar mundo visible
    width=t.width;
    height=t.height;
...
var gravityFieldMode = 0; // 0 = off, 1 = arrows, 2 = grid
...
function toggleOrbits() {
    ...
    // The path should *not* be cleared here when hiding, as it would erase the history.
    // It should only be cleared if you want to reset the paths completely,
    // which typically happens when the user wants to start fresh.
    // For "trazarse desde el momento que se activen y no debe borrarse hasta que se desactiven",
    // we remove the path clearing from here.
}

// NEW: Function to clear all particle orbit paths
function clearAllParticleOrbits() {
    for (var i = 0; i < particleList.length; i++) {
        particleList[i].path = []; // Clear the path for all particles
    }
}
...
// NEW: Toggles particle mass display
function toggleParticleMass() {
    showParticleMass = !showParticleMass; // Toggle the state
}
            ...
            for(var f=u+1;f<particleList.length;f++){
                if(!p[f]){
                    ...
                    if(b<m.radius/1.5+g.radius/1.5){
                        ...
                        c.push(new Particle(w,x,P,C,A,m.type==g.type?m.type:"matter","Merged Particle")); // Pass empty name for merged
                        break
                    }
                }
            }
...
function draw(){
    ...
    if(gravityFieldEnabled){
        ...
        var lineWidth = 1 / zoomLevel; // Ancho de línea ajustado al zoom
        ...
        // Dibujar líneas horizontales de la cuadrícula (deformadas)
        for(var y = worldViewTop; y < worldViewBottom; y += gridSize){
            var points = [];
            // Calcular puntos a lo largo de esta línea horizontal
            for(var x = worldViewLeft; x <= worldViewRight; x += 5){
                var displacedX = x;
                var displacedY = y;
                
                // Aplicar desplazamiento por gravedad en cada punto
                for(var i = 0; i < particleList.length; i++){
                    var p = particleList[i];
                    var dx = p.x - x;
                    var dy = p.y - y;
                    ...
                    if(distSq < 10000 && distSq > 100){ // Evitar divisiones por cero y efectos demasiado cercanos
                        var dist = Math.sqrt(distSq);
                        var force = p.mass / distSq * (p.type === "matter" ? 1 : -1);
                        var displacement = Math.min(20, force * 1000 / dist); // Limitar el desplazamiento
            ...
            // Dibujar la línea horizontal deformada
            if(points.length > 1){
        ...
        // Dibujar líneas verticales de la cuadrícula (deformadas)
        context.beginPath();
        context.strokeStyle = "rgba(100, 100, 255, 0.5)";
        context.lineWidth = lineWidth;
        ...
        for(var x = worldViewLeft; x < worldViewRight; x += gridSize){
            var points = [];
            // Calcular puntos a lo largo de esta línea vertical
            ...
            // Dibujar la línea vertical deformada
            if(points.length > 1){
                context.moveTo(points[0].x, points[0].y);
                for(var j = 1; j < points.length; j++){
                    context.lineTo(points[j].x, points[j].y);
        ...
        // Opcional: Dibujar puntos de la cuadrícula en las intersecciones originales
        context.beginPath();
    ...
    // NEW: Draw Black Hole event horizon
    for (var i = 0; i < particleList.length; i++) {
        var p = particleList[i];
        if (p.isBlackHole) { // Check if the particle is a black hole
            // Simplified Schwarzschild radius calculation for visualization purposes
            // In a real simulation, this would involve G (gravitational constant) and c (speed of light)
            // Rs = 2GM/c^2. Here we use a scaled version.
            var schwarzschildRadius = p.mass / 1000; // Adjust this scaling factor as needed

            // Draw the event horizon
            context.beginPath();
            context.arc(p.x, p.y, schwarzschildRadius, 0, 2 * Math.PI);
            context.closePath();
            context.fillStyle = "rgba(50, 50, 50, 0.7)"; // Dark grey, semi-transparent
            context.fill();
            context.strokeStyle = "rgba(100, 100, 100, 0.9)"; // Slightly lighter grey border
            context.lineWidth = 2 / zoomLevel; // Scale line width with zoom
            context.stroke();

            // Draw "Black Hole" text
            context.fillStyle = "#ffffff";
            context.font = (12 / zoomLevel) + "px Arial";
            context.textAlign = "center";
            context.fillText("Black Hole", p.x, p.y - schwarzschildRadius - (10 / zoomLevel)); // Adjust offset
        }
    }
    ...
    for(var t=0;t<particleList.length;t++){
        ...
        // NEW: Draw particle mass if enabled
        if(showParticleMass){
            context.fillStyle="#ffffff"; // Color del texto
            context.font=(8 / zoomLevel) + "px Arial"; // Tamaño de fuente ligeramente más pequeño para la masa
            context.textAlign="center"; // Alineación del texto
            // Muestra la masa en notación científica o un número fijo si es pequeño
            var massText = e.mass < 10000 ? e.mass.toFixed(0) : e.mass.toExponential(1);
            context.fillText("Mass: " + massText, e.x, e.y + e.radius + (showParticleNames ? 15 / zoomLevel : 5 / zoomLevel)); // Ajusta el offset Y
        }
    }
