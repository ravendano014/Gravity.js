...
function mouseUpListener(t) {
    window.removeEventListener("mousemove", mouseMoveListener),
        ...
        !t.shiftKey && !shiftDown && particleList.push(new Particle(newMass, startCoords[0], startCoords[1], endCoords[0] - startCoords[0], endCoords[1] - startCoords[1], currentParticleType)),
...
function toggleParticleType() {
    (currentParticleType = currentParticleType == "matter" ? "antimatter" : "matter"), (document.getElementById("particleTypeBtn").innerHTML = currentParticleType == "matter" ? "Matter" : "Antimatter");
}
function integrate() {
            ...
            for (var f = u + 1; f < particleList.length; f++) {
                if (!p[f]) {
                    ...
                    if (b < m.radius / 1.5 + g.radius / 1.5) {
                        (p[u] = !0), (p[f] = !0);
                        var w = m.mass + g.mass,
                            x = (m.x * m.mass + g.x * g.mass) / w,
                            P = (m.y * m.mass + g.y * g.mass) / w,
                            C = (m.vx * m.mass + g.vx * g.mass) / w,
                            A = (m.vy * m.mass + g.vy * g.mass) / w;
                        c.push(new Particle(w, x, P, C, A, m.type == g.type ? m.type : "matter"));
                        break;
                    }
                }
            }
...
function draw() {
    ...
    for (var t = 0; t < particleList.length; t++) {
        var e = particleList[t],
            o = context.createRadialGradient(e.x, e.y, 0.75 * e.radius, e.x, e.y, e.radius);
